...
namespace Framework
{
    public partial class FsmManager
    {
        /// <summary>
        /// 有限状态机。
        /// </summary>
        /// <typeparam name="T">有限状态机持有者类型。</typeparam>
        private sealed class Fsm<T> : FsmBase, IFsm<T> where T : class
        {
            private T m_Owner;
            ...
            public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
            {
                ...
                foreach (FsmState<T> state in states)
                {
                    ...
                    {
                        throw new Exception(string.Format("FSM '{0}.{1}' state '{2}' is already exist.", typeof(T), name, stateType));
                    }
            ...
            public void Start<TState>() where TState : FsmState<T>
            {
                ...
                if (state == null)
                {
                    throw new Exception(string.Format("FSM '{0}.{1}' can not start state '{2}' which is not exist.", typeof(T), Name, typeof(TState).FullName));
                }
            ...
            public void Start(Type stateType)
            {
                ...
                if (state == null)
                {
                    throw new Exception(string.Format("FSM '{0}.{1}' can not start state '{2}' which is not exist.", typeof(T), Name, stateType.FullName));
                }
            ...
            public void ChangeState(Type stateType)
            {
                ...
                if (state == null)
                {
                    throw new Exception(string.Format("FSM '{0}.{1}' can not change state to '{2}' which is not exist.", typeof(T), Name, stateType.FullName));
                }
